...
public class PrescriptionController {
    ...
    public String patientPrescriptions(Model model, Authentication authentication) {
        ...
        java.util.List<com.medicare.clinical.model.Patient> matchingPatients = new java.util.ArrayList<>();
        ...
        java.util.List<com.medicare.clinical.model.Medicine> userMedicines = new java.util.ArrayList<>();
        java.util.List<MedicationNotification> userNotifications = new java.util.ArrayList<>();

        for (com.medicare.clinical.model.Patient p : matchingPatients) {
            if (p != null && p.getId() != null && !p.getId().isBlank()) {
                ...
                for (com.medicare.clinical.model.Medicine m : medicineRepository.findByPatientId(p.getId())) {
        ...
        for (String key : searchKeys) {
            ...
            for (com.medicare.clinical.model.Medicine m : medicineRepository.findByPatientPatientUsername(key)) {
                if (m != null && m.getId() != null && userMedicines.stream().noneMatch(x -> m.getId().equals(x.getId()))) {
                    userMedicines.add(m);
                }
            }
            for (com.medicare.clinical.model.Medicine m : medicineRepository.findByPatientEmailIgnoreCase(key)) {
                if (m != null && m.getId() != null && userMedicines.stream().noneMatch(x -> m.getId().equals(x.getId()))) {
                    userMedicines.add(m);
                }
            }
